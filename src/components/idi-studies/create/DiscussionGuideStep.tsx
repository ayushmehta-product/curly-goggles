...
import { MOCK_DISCUSSION_GUIDE_SECTIONS } from '@/data/mock-discussion-guide';
...
function createId() {
  return `question-${Date.now()}-${Math.round(Math.random() * 1000)}`;
}
...
function createInitialQuestions(): ModerationQuestion[] {
  let startMinutes = 0;

  return MOCK_DISCUSSION_GUIDE_SECTIONS.flatMap((section) =>
    section.questions.map((question) => {
      ...
      startMinutes += question.durationMinutes;
      return moderationQuestion;
    })
  );
}

const TIMESTAMP_OPTIONS: TimestampOption[] = Array.from({ length: 91 }, (_, minutes) => ({
  value: String(minutes),
  label: formatTimestamp(minutes),
}));

const ACTIVITY_LOG_ACCESSORS = {
  id: 'id',
  date: 'date',
  time: 'time',
  userName: 'userName',
...
function getNextStartMinutes(questions: ModerationQuestion[]) {
  if (questions.length === 0) return 0;
  return Math.min(90, Math.max(...questions.map((question) => question.startMinutes)) + 5);
}

function getSortedQuestions(questions: ModerationQuestion[]) {
  return [...questions].sort((firstQuestion, secondQuestion) => {
    if (firstQuestion.startMinutes !== secondQuestion.startMinutes) {
      return firstQuestion.startMinutes - secondQuestion.startMinutes;
    }

    return questions.indexOf(firstQuestion) - questions.indexOf(secondQuestion);
  });
}

function getSelectedTimestamp(startMinutes: number) {
  return TIMESTAMP_OPTIONS.find((option) => option.value === String(startMinutes)) ?? TIMESTAMP_OPTIONS[0];
}
...
export function DiscussionGuideStep({ onBack, onSaveDraft, onContinue }: DiscussionGuideStepProps) {
  const [questions, setQuestions] = useState<ModerationQuestion[]>(createInitialQuestions);
  ...
  const timelineLogs: TimelineLog[] = displayQuestions.map((question) => ({
    id: question.id,
    date: '',
    time: formatTimestamp(question.startMinutes),
  ...
  function openEditQuestion(question: ModerationQuestion) {
    setQuestionForm({
      mode: 'edit',
      id: question.id,
      question: question.question,
  ...
  function saveQuestion() {
    ...
    const questionText = questionForm.question.trim() || 'Untitled interview question.';

    if (questionForm.mode === 'edit' && questionForm.id) {
      setQuestions((currentQuestions) =>
        currentQuestions.map((question) =>
          question.id === questionForm.id
            ? { ...question, question: questionText, startMinutes: questionForm.startMinutes }
            : question
        )
      );
    } else {
      setQuestions((currentQuestions) => [
        ...currentQuestions,
        {
          ...
          startMinutes: questionForm.startMinutes,
        },
      ]);
    }
  ...
  return (
    <div className="mx-auto w-full max-w-4xl">
      <section className="bg-white px-8 py-10">
        <div className="mb-10">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-500">
            A focused live moderation outline for the interview.
          </p>
        </div>
        ...
        {questions.length === 0 ? (
          <div className="border-t border-gray-100 py-12">
            ...
            <p className="mt-2 text-sm text-gray-500">Start building your moderation outline.</p>
            <div className="mt-6">
          ...
          <div className="space-y-8">
            <div className="relative">
              <div aria-hidden="true" className="hidden">
                <WuActivityLog logs={timelineLogs} accessorKey={ACTIVITY_LOG_ACCESSORS} />
              </div>
      ...
      <QuestionEditorModal
        ...
        onChange={(updates) => {
          setQuestionForm((currentForm) => (currentForm ? { ...currentForm, ...updates } : currentForm));
        }}
        onSave={saveQuestion}
      />
    </div>
